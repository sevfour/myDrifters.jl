...
begin	
	using Drifters, PlutoUI, StatsPlots
	import Climatology
...
$(@bind tt Slider(1:2; default=2, show_value=true))
"""
...
begin	
	...
	I=Individuals(P,df.x,df.y,fill(z_init,n_part),df.f,
		(🔴=deepcopy(OCCAmodule.custom🔴),🔧=OCCAmodule.custom🔧, D=D))
...
begin
	...
	🔴_by_ID = DataFrames.groupby(I.🔴, :ID)
	🔴_by_t = DataFrames.groupby(I.🔴, :t)
	
	nt=length(unique(I.🔴.t))

	ff(x) = x[1]
	tmp1=DataFrames.combine(🔴_by_ID,:lon => ff => :lo,:lat => ff => :la)
	I.🔴.dlon=I.🔴.lon - tmp1.lo[I.🔴.ID]
	I.🔴.dlat=I.🔴.lat - tmp1.la[I.🔴.ID]
...
begin
	...
	#f(x)=x[tt]-x[1]
	#f(x)=last(x).-first(x)
	#cdf = DataFrames.combine(🔴_by_ID,nrow,:lat => f => :dlat,:lon => f => :dlon)

	g(x)=x[tt]
	cdf = DataFrames.combine(🔴_by_ID,:dlat => g => :dlat,:dlon => g => :dlon)
...
begin	
	xx=vec(D.Γ.XC[1][:,1])
	yy=vec(D.Γ.YC[1][1,:])
	zz=transpose(log10.(D.Γ.Depth[1]))
	𝐵=(x = xx, y = yy, z = zz)
...
manifest_format = "2.0"
project_hash = "599f4e163858157f213af8c1f35c570e0162870c"
